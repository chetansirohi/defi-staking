...
function App() {
  ...
  //wallet connection, if not undefined wallet is connected
  const isConnected = () => signer !== undefined

  //connect wallet button is clicked. user can call the contract and methods that write onto a blockchain
  const getSigner = async () => {
    provider.send("eth_requestAccounts", [])
    const signer = provider.getSigner()
    return signer
  }
  //takes the address of wallet that call and signer
  const getAssetIds = async (address, signer) => {
    const assetIds = await contract.connect(signer).getPositionIdsForAddress(address)
    return assetIds
  }
   //calculate the days till unlock
  const calcDaysRemaining = (unlockDate) => {
    const timeNow = Date.now() / 1000
    const secondsRemaining = unlockDate - timeNow
    return Math.max( (secondsRemaining / 60 / 60 / 24).toFixed(0), 0)
  }
  //takes the id of the asset,loop through all the ids 
  const getAssets = async (ids, signer) => {
    const queriedAssets = await Promise.all(
      ids.map(id => contract.connect(signer).getPositionById(id))
    )
    //create an object to work with asset data that comes back from getPositionById
   ...
   //gets called when user clicks the connect button,queries the assets or positiosn for the currently connected wallet
  ...
  //stake ether , convert ether to wei
  const stakeEther = () => {
